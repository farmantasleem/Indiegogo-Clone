...
export default function Projects() {
  ...
  return (<Container className="projects" minW="100%" p={"10px"} >
    <Stack borderBottom="2px" pb={"10px"} borderColor={"rgb(42,42,42)"} w="full" direction={{ base: "column", md: "row" }}>
      <Image height="400px" w={{ base: "100%", md: "50%" }} src={gdata.img} />
      <VStack pl={"20px"} gap={{ base: "10px", md: "20px" }} w={{ base: "100%", md: "50%" }} textAlign={"left"}>
        <Text w={"full"}>{gdata.category}</Text>
        <Heading>{gdata.name}
        </Heading>
        <Text>
         {gdata.desc}</Text>
        <Heading w={"full"} textAlign={"left"}>{gdata.prc} by 3,910 backers</Heading>
        <HStack w="full" textAlign={"left"}> <Button _hover={{ color: "white", bgColor: "silver" }} color={"white"} bgColor={"rgb(229,16,117)"}>Make a Contribution</Button> <Button>Follow</Button><HStack>
          <Button bgColor={"white"} colorScheme='facebook' leftIcon={<FaFacebook />}>
            Share
          </Button>
          <Button bgColor={"white"} colorScheme='twitter' leftIcon={<FaTwitter />}>
            Share
          </Button>
        </HStack></HStack>
      </VStack>
    ...
    <Stack direction={{ base: "column", md: "row" }} w="100%" mt={"20px"}>
      ...
      <Tabs minW={{ base: "100%", md: "49.5%" }} maxW={{ base: "100%", md: "49.5%" }} onChange={(index) => setTabIndex(index)} bg={bg}>
        ...
        <TabPanels p='2rem'>
          <TabPanel textAlign={"left"} width={{ base: "100%", md: "100%" }}>What is Lorem Ipsum?
          ...
          Where does it come from?
          Contrary to popular belief, Lorem Ipsum is not simply random text. It has roots in a piece of classical Latin literature from 45 BC, making it over 2000 years old. Richard McClintock, a Latin professor at Hampden-Sydney College in Virginia, looked up one of the more obscure Latin words, consectetur, from a Lorem Ipsum passage, and going through the cites of the word in classical literature, discovered the undoubtable source. Lorem Ipsum comes from sections 1.10.32 and 1.10.33 of "de Finibus Bonorum et Malorum" (The Extremes of Good and Evil) by Cicero, written in 45 BC. This book is a treatise on the theory of ethics, very popular during the Renaissance. The first line of Lorem Ipsum, "</TabPanel>
          <TabPanel textAlign={"left"} width={{ base: "100%", md: "100%" }}>
